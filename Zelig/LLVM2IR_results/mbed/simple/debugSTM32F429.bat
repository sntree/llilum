...
kill openocd.exe
START /I openocd.exe -f %LLILUM_OPENOCD_SCRIPTS%interface\stlink-v2-1.cfg -f %LLILUM_OPENOCD_SCRIPTS%board\stm32f429disc1.cfg

PAUSE

ECHO.
ECHO Trying to attach to remote target on port 3333
"%GCC_BIN%arm-none-eabi-gdb.exe" STM32F429\mbed_simple.elf %* -ex "target remote :3333" -ex "monitor halt reset" -ex "ni"
